#!/usr/bin/env python3
"""
PMM V2 Cookbook - Ethereum Sepolia

This is the testnet companion to `pmm_cookbook_mainnet.py`.
...
RPC_URL = os.getenv("SEPOLIA_RPC_URL", "https://ethereum-sepolia.publicnode.com")
CHAIN_ID = 11155111
USDC_ADDRESS = Web3.to_checksum_address(
    os.getenv("USDC_ADDRESS", "0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238")
)
...
class PMMV2Cookbook:
    ...
    def mint_mock_usdc(self, amount: int):
        return self._send(self.usdc.functions.mint(self.account.address, amount))
...
def main():
    parser = argparse.ArgumentParser(description="PMM V2 Ethereum Sepolia cookbook")
    ...
    mint = sub.add_parser("mint-mock-usdc")
    mint.add_argument("amount", type=float, help="Mock USDC amount")
    ...
    elif args.command == "mint-mock-usdc":
        print_result(cookbook.mint_mock_usdc(int(args.amount * cookbook.usdc_unit)))
