...
describe("PMM V2 - Proof Of Proximity", function () {
  ...
  it("uses nMax to determine pairwise connections", async function () {
    ...
    await pmm.connect(alice).createAgent(agentId("charlie"), 18, 24);
    await pmm.connect(alice).createAgent(agentId("david"), 21, 28);
    await pmm.connect(alice).createAgent(agentId("eric"), 15, 20);
  ...
  it("records used puzzle destinations directly", async function () {
    const { pmm, alice } = await deployV2();
    const id = agentId("used-destination-direct");
  ...
  it("keeps nMax monotonic across smaller and larger later solutions", async function () {
    const { pmm, tbn, alice } = await deployV2();
    const mover = agentId("monotonic-mover");
    const helper = agentId("monotonic-helper");
    ...
    await pmm.connect(alice).createAgent(agentId("smaller-solution"), 15, 20); // n = 5
    expect(await pmm.nMax()).to.equal(6);

    // Add c=5 to set TVL=105, then move c=10 -> c=74 for n=8 and TVL=169.
    await pmm.connect(alice).createAgent(agentId("tvl-adjuster"), 3, 4);
