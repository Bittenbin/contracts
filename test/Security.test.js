...
describe("Security and Exploit Prevention", function () {
  ...
  describe("Yield Exploitation Prevention", function () {
    ...
    it("Should have constant emission rate (no market count gaming)", async function () {
      // Create multiple markets - emission rate stays constant
      const rateBefore = await pmm.REWARD_RATE();
      
      for (let i = 0; i < 5; i++) {
        await pmm.connect(alice).createMarket(i + 1, 3 + i, 4 + i);
      }
      
      const rateAfter = await pmm.REWARD_RATE();
      // Rate is constant - no manipulation possible
      expect(rateAfter).to.equal(rateBefore);
      expect(rateAfter).to.equal(31709n); // ~0.0317 TENBIN/second
    });

    it("Should prevent claiming rewards from non-existent position", async function () {
      await pmm.connect(alice).createMarket(PLATFORM_ID, 3, 4);
      
      // Attacker never participated but tries to claim
      const balanceBefore = await tenbin.balanceOf(attacker.address);
      await pmm.connect(attacker).claimRewards();
      const balanceAfter = await tenbin.balanceOf(attacker.address);
    ...
    it("Should prevent double claiming same rewards", async function () {
      await pmm.connect(alice).createMarket(PLATFORM_ID, 3, 4);
      ...
      // Should only earn small amount for 1 block (31709)
      const diff = balanceAfterSecond - balanceAfterFirst;
      expect(diff).to.be.closeTo(31709n, 100n); // 1 second of rewards
    });
  });
