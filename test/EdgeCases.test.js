...
describe("Edge Cases and Security Tests", function () {
  ...
  describe("Staking Rewards Edge Cases", function () {
    beforeEach(async function () {
      await tenbin.setMinter(await pmm.getAddress());
    });

    it("Should handle consecutive reward claims", async function () {
      await pmm.connect(alice).createMarket(PLATFORM_ID, 3, 4);
      ...
      await pmm.connect(alice).claimRewards();
      const balance2 = await tenbin.balanceOf(alice.address);
      const firstClaim = balance2 - balance1;
      expect(firstClaim).to.be.gt(0);
      
      // Immediate second claim (next block)
      await pmm.connect(alice).claimRewards();
      const balance3 = await tenbin.balanceOf(alice.address);
      
      // Should earn small amount for 1 block
      const diff = balance3 - balance2;
      expect(diff).to.be.closeTo(31709n, 100n);
      
      // Wait more time
      await ethers.provider.send("evm_increaseTime", [30 * 24 * 60 * 60]);
      await ethers.provider.send("evm_mine", []);
      
      // Third claim should have rewards again
      await pmm.connect(alice).claimRewards();
      const balance4 = await tenbin.balanceOf(alice.address);
      expect(balance4).to.be.gt(balance3);
    });

    it("Should aggregate stake across multiple platforms for same user (global rewards)", async function () {
      const PLATFORM_2 = 9999999999;
      
      // Create two markets
      await pmm.connect(alice).createMarket(PLATFORM_ID, 3, 4); // 5 tokens stake
      await pmm.connect(alice).createMarket(PLATFORM_2, 5, 12); // 13 tokens stake
      
      // User's total stake should be aggregated
      const aliceStake = await pmm.userTotalStake(alice.address);
      expect(aliceStake).to.equal(ethers.parseUnits("18", 6)); // 5 + 13
      
      // Advance time
      await ethers.provider.send("evm_increaseTime", [30 * 24 * 60 * 60]);
      await ethers.provider.send("evm_mine", []);
      
      // Single claim gets rewards from all platforms (global pool)
      const balance1 = await tenbin.balanceOf(alice.address);
      await pmm.connect(alice).claimRewards();
      const balance2 = await tenbin.balanceOf(alice.address);
  ...
  describe("Holdings and Cost Basis Direct Access", function () {
    it("Should return correct holdings struct values", async function () {
      ...
      // Verify global stake is also updated
      const aliceStake = await pmm.userTotalStake(alice.address);
      expect(aliceStake).to.equal(5000000n); // 4 + 1 = 5 TENBIN
    });
...
describe("TENBIN Token Edge Cases", function () {
  ...
  it("Should handle very large minting amounts up to max supply", async function () {
    const remaining = await tenbin.remainingMintableSupply();
    // Should succeed
    await tenbin.mint(alice.address, remaining);
    
    // Should fail if exceeding
    await expect(tenbin.mint(alice.address, 1))
